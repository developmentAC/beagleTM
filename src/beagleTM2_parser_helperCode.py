...
def helper():
    ...
    command_str = printWithColour(
        BIGreen, f"\t [+] \U0001f600 USAGE: ./{THISPROG}  keywordFile.md"
    )
    ...
    print(
        BIGreen
        + f"\t [+] The INPUT directory (Data files are located here)  : {CORPUS_DIR}"
        + White
    )
    print(
        BIGreen
        + f"\t [+] The OUTPUT directory (Output data is placed here)  : {MYOUTPUT_DIR}"
        + White
    )
    print("\t" + len(h_str2) * "-")
    print(
        "\tNotes. Data can be downloaded from: ftp://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/"
    )
    print(
        BIYellow
...
def saveStats(stats_dic, inFile0):
    """Function to save the key word percentages statistics generated by processing articles."""
...
def makeCSVFile(in_list, inFile0_str):
    ...
    file = open(
        inFile1_str, "w", newline=""
    )  # the newline should make this cross platform compatible
